...
const Modal = ({closeModal, data}) => {
  ...
  currentDate = currentDate.toLocaleDateString('default', {year: 'numeric', day: 'numeric', month: 'long'});
...
const DateModal = ({closeModal, data}) => {
  return (
    <>
      {ReactDOM.createPortal(<BackDrop closeModal={closeModal}></BackDrop>, document.getElementById('overlays'))}
      {ReactDOM.createPortal(<Modal data={data} closeModal={closeModal}></Modal>, document.getElementById('overlays'))}
    </>
  )
}
